import React from 'react';

function Promises() {
    return (
        <div id='whyus' className=' px-5 mt-16 lg:mt-20 lg:flex justify-between items-center content-center lg:px-20'>
            <div className='max-w-2xl'>
            <p className='text-base lg:text-lg pill tracking-wider bg-blue-100 w-36 lg:w-40 px-4 py-2 rounded-full'>Our Promises</p>
            <p className="heading-color font-bold text-4xl pt-3 lg:text-6xl ">We Promise to Always Try and Give The Best</p>
            </div>
            <div>
            <p className="text-gray-600 pt-4 lg:text-xl">We will deliver the best services for family to make them keep protected</p>           
            </div>
        </div>
    )
}

export default Promises
